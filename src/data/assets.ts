...
export const assetKey = (path: string) =>
  path
    .split('/')
    .pop()!
...
const availablePhotoUrls = new Set(photosByKey.values());

export const photo = (name: string) =>
  photosByKey.get(assetKey(name)) ??
  withBase(`/assets/photos/${name}.png`);

export const hasPhoto = (src: string) =>
  availablePhotoUrls.has(src);
...
export const photoFraming: Record<string, Framing> = {
  ...
  scene2week1: { position: '50% 20%' },
  scene2week3: { position: '55% 20%' },
  scene2week5: { position: '50% 14%' },
  scene2graduation: { position: '50% 12%' },

  scene3her: { position: '50% 50%', ratio: '941 / 1672' },
  scene3me: { position: '50% 50%', ratio: '941 / 1672' },
